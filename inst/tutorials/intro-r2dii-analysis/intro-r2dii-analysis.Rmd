...
```{r match-prioritize, exercise=TRUE}
...
```{r target-market-share, exercise=TRUE}
matched %>%
  target_market_share(
    ald = ald_demo,
...
```{r target-market-share-by-company, exercise=TRUE}
matched %>%
  target_market_share(
    ald = ald_demo,
    scenario = scenario_demo_2020,
...
```{r join-ald-scenario, exercise=TRUE}
...
```{r summarize-weighted-production, exercise=TRUE}
# portfolio level
