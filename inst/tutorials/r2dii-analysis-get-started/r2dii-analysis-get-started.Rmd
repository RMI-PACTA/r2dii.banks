...
## Get started with r2dii.analysis
...
```{r use-r2dii, exercise.setup='setup'}
library(r2dii.data)
library(r2dii.match)
...
```{r use-tidyverse, exercise.setup='use-r2dii'}
library(tidyverse)
```
...
```{r matched, exercise.setup='use-tidyverse'}
...
```{r market-share-targets-portfolio, exercise.setup='matched'}
# Use these datasets to practice but eventually you should use your own data.
...
```{r share-company-define, exercise.setup='market-share-targets-portfolio'}
...
```{r share-company-result, exercise.setup='share-company-define'}
...
```{r sda-targets-define, exercise.setup='share-company-result'}
...
```{r sda-targets-result, exercise.setup='sda-targets-define'}
sda_targets
```
...
```{r technoloy-mix-portfolio, exercise.setup='sda-targets-result'}
...
```{r plot-metric-versus-weighted-capacity, exercise.setup='technoloy-mix-portfolio'}
ggplot(
  data = tech_mix_data,
  mapping = aes(
    x = metric,
...
```{r targets-and-ceiling-1, exercise.setup='plot-metric-versus-weighted-capacity', exercise.setup='just-targets', exercise.setup='just-data'}
...
```{r renewables-data, exercise.setup='targets-and-ceiling-1'}
...
```{r just-targets, exercise.setup='renewables-data'}
just_targets <- renewables_data %>%
  filter(str_detect(metric, "target_"))
```


```{r ceiling, exercise.setup='just-targets'}
...
```{r just-data, exercise.setup='ceiling'}
just_data <- renewables_data %>%
  filter(!str_detect(metric, "target_"))
```

```{r targets-and-ceiling, exercise.setup='just-targets', exercise.setup='just-data'}
...
```{r plot-year-versus-weithed-production-value, exercise.setup='targets-and-ceiling'}
ggplot(
...
```{r sda-plot, exercise.setup='plot-year-versus-weithed-production-value'}
ggplot(
  data = sda_targets,
  mapping = aes(
    x = year,
